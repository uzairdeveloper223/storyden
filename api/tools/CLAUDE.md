...
After modifying `robots.yaml`, regenerate the Go code:

```bash
go generate ./api/...
```

This generates `api/mcp_schema.go` with type-safe Go structs.

### 12.3 Related files

- `api/CLAUDE.md` - These guidelines (you are here)
- `api/robots.yaml` - Tool schema definitions
- `api/mcp_schema.go` - Generated Go code (do not edit)
- `api/generate.go` - Code generation directive
- `api/embed.go` - Schema embedding for runtime
